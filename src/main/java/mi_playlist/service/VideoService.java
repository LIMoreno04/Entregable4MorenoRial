...
public class VideoService {
    ...
    private void fetchYouTubeStats(String videoId, VideoModel video) {
        try {
            ...
            String apiUrl = buildYouTubeApiUrl(videoId, apiKey);

            ResponseEntity<String> response = callYouTubeApi(apiUrl);
            if (response != null && response.getStatusCode().is2xxSuccessful()) {
                String jsonResponse = response.getBody();
                boolean parsed = parseYouTubeApiResponse(jsonResponse, video);
                if (parsed) {
    ...
    private String buildYouTubeApiUrl(String videoId, String apiKey) {
        return "https://www.googleapis.com/youtube/v3/videos?part=statistics&id=" + videoId + "&key=" + apiKey;
    }

    private ResponseEntity<String> callYouTubeApi(String apiUrl) {
        return restTemplate.getForEntity(apiUrl, String.class);
    }

    private boolean parseYouTubeApiResponse(String jsonResponse, VideoModel video) {
        try {
